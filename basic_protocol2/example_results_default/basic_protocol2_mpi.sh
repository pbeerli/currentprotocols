...
echo " all 4 examples for the basic protocol 2"
#mkdir -p $tempdir
#cd $tempdir
...
nohup mpirun -np 9 --oversubscribe migrate-n-mpi  parmfile_model1 -nomenu > parm_model1.log 2>parm_model1.err
time=`date`
echo "$time migrate-n parmfile_model2 running now"
nohup mpirun -np 9 --oversubscribe migrate-n-mpi  parmfile_model2 -nomenu > parm_model2.log 2> parm_model2.err
time=`date`
